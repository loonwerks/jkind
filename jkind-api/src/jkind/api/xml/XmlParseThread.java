...
public class XmlParseThread extends Thread {
	...
	private void readFunction(Counterexample cex, Element functionElement) {
		String name = functionElement.getAttribute("name");
		List<VarDecl> inputs = getFunctionParameters(functionElement, "Input");
		FunctionTable table = cex.getOrCreateTable(name, inputs);
		
		List<VarDecl> outputs = getFunctionParameters(functionElement, "Output");
		for (VarDecl output : outputs) {
			table.addOutput(output);
		}
		
		readFunctionValues(table, inputs, outputs, getElements(functionElement, "FunctionValue"));
	}

	private List<VarDecl> getFunctionParameters(Element functionElement, String classifier) {
		List<VarDecl> params = new ArrayList<>();
		for (Element inputElement : getElements(functionElement, classifier)) {
			String name = inputElement.getAttribute("name");
			Type type = parseType(inputElement.getAttribute("type"));
			params.add(new VarDecl(name, type));
		}
		return params;
	}
	
	private void readFunctionValues(FunctionTable table, List<VarDecl> inputs,
			List<VarDecl> outputs, List<Element> functionValueElements) {
		List<VarDecl> params = new ArrayList<>();
		params.addAll(inputs);
		params.addAll(outputs);
		
		for (Element functionValueElement : functionValueElements) {
			List<Value> values = new ArrayList<>();
			List<Element> args = getElements(functionValueElement, "ArgumentValue");
			for (int i = 0; i < args.size(); i++) {
				values.add(getValue(args.get(i), params.get(i).type.toString()));
			}
			table.setOutputs(values.subList(0, inputs.size()), values.subList(inputs.size(), values.size()));
		}
	}
