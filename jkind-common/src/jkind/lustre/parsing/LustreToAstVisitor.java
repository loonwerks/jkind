...
public class LustreToAstVisitor extends LustreBaseVisitor<Object> {
    ...
    public Program program(ProgramContext ctx) {
        ...
        List<RepairNode> repairNodes = repairNodes(ctx.repairnode());
        return new Program(loc(ctx), types, constants, functions, nodes, repairNodes, main);
    }
    ...
    private List<RepairNode> repairNodes(List<LustreParser.RepairnodeContext> ctxs) {
        return ctxs.stream().map(this::repairNode).collect(toList());
    }
    ...
    public RepairNode repairNode(LustreParser.RepairnodeContext ctx) {
        String id = ctx.ID().getText();
        List<VarDecl> inputs = varDecls(ctx.input);
        List<VarDecl> holeinputs = varDecls(ctx.holeinput);
        List<VarDecl> outputs = varDecls(ctx.output);
        List<VarDecl> locals = varDecls(ctx.local);
        List<Equation> equations = equations(ctx.equation());
        List<Expr> assertions = assertions(ctx.assertion());
        ...
        return new RepairNode(loc(ctx), id, inputs, holeinputs, outputs, locals, equations, assertions, contract);
    }
    ...
    @Override
    public Expr visitRepairExpr(LustreParser.RepairExprContext ctx) {

        Expr left = expr(ctx.expr(0));
        Expr right = expr(ctx.expr(1));
        assert(right instanceof NodeCallExpr);

        return new RepairExpr(loc(ctx), left, (NodeCallExpr) right);
    }
